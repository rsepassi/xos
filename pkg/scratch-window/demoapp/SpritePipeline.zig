...
pub fn makeArgs(self: @This(), spritesheet: SpriteSheet, sprite_locs: SpriteLocs) SpritePipelineArgs {
...
pub const SpritePipelineArgs = struct {
    ...
    fn init(pipeline: SpritePipeline, spritesheet: SpriteSheet, locs: SpriteLocs) @This() {
        return .{
            .vertices = locs.pos,
            .nvertices = locs.nvertices,
...
pub fn run(self: @This(), pass: gpu.RenderPassEncoder, args: SpritePipelineArgs) !void {
    if (args.nvertices == 0) log.warn("nvertices=0", .{});
    ...
    pass.draw(.{ .vertex_count = args.nvertices });
}
